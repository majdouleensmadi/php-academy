...
// Retrieve the employee leave data
if (isset($_POST['submit'])) {
    ...
    if ($amount>14) {
            ...
            $sql = "UPDATE employee SET salary = salary /30 * $amount WHERE id=$id";
            $statement = $mysqli->prepare($sql);
            $statement->execute();
    
            echo "successful";
    }else{
        $sql = "UPDATE employee SET off_days = offdays-$amount  WHERE id=$id";
        $statement = $mysqli->prepare($sql);
        $statement->execute();
        echo "not update" ;
    }
